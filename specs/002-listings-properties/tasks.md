...
- [ ] T015 [P] [US2] Write tests for OptionsFlowHandler: test `async_step_init` presents `scan_interval` field with `DEFAULT_SCAN_INTERVAL` default, test valid interval saves to `entry.options` and creates entry, test interval below `MIN_SCAN_INTERVAL` raises validation error, test config flow class exposes `OPTIONS_FLOW` in tests/test_config_flow.py
